...
function Timer({ ...props }: ITimerComponentProps) {
  const { isStarted, setIsStarted, timerSeconds, setTimerSeconds, timerIntervalIdRef } = props;
    ...
    <>
      ...
      <InnerWrapperBlockStyles>
        <CancelControlButtonStyles
          onClick={handleReset}
          title={'Сброс'}
          disabled={!timerSeconds || isStarted}
          backgroundColor1="white"
        />
        {!isStarted && (
          <StartControlButtonStyles
            onClick={handleStart}
            title={!timerSeconds ? 'Старт' : 'Дальше'}
            backgroundColor1="white"
          />
        )}
